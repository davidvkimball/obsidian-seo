...
A comprehensive SEO analysis plugin for Obsidian that helps optimize notes for search engines and AI. Part of the [Vault CMS](https://github.com/davidvkimball/vault-cms) project, this plugin provides both current note and vault-wide SEO auditing capabilities.
...
- **Comprehensive SEO Checks**:
  - Content Quality: Content length, reading level, keyword density, duplicate content detection
  - Technical SEO: Title optimization, meta descriptions, heading structure, alt text validation
  - Link Management: Broken links, external links, naked links, potentially broken links/embeds
- **User Experience**: Flexible sorting, one-click navigation, intelligent caching, configurable settings

The plugin uses a weighted scoring system (40-100 range) that prioritizes critical SEO factors, with caching for performance optimization.

### Important Project-Specific Details

- **Project Type**: Plugin
- **Purpose**: SEO auditing and optimization for Obsidian notes, particularly for public-facing content
- **Status**: Production (version 0.3.4)
- **Min App Version**: 0.15.0
- **Desktop Only**: No (works on mobile)

**Architecture**:
- **Modular Check System** (`src/checks/`) - Individual check implementations for different SEO aspects
- **UI Panel System** (`src/ui/`) - Side panel views for current note and vault-wide audits
- **Settings Management** (`src/settings.ts`, `src/settings-tab.ts`) - Settings interface and UI with SettingGroup support
- **Cache Management** (`src/utils/cache-manager.ts`) - Performance optimization with 24-hour cache expiration
- **Error Handling** (`src/utils/error-handler.ts`) - Centralized error handling utilities
- **Performance Utilities** (`src/utils/performance.ts`) - Performance monitoring and optimization

**Key Features**:
- Current note and vault-wide SEO auditing
- Content quality checks (length, reading level, keyword density, duplicates)
- Technical SEO (title optimization, meta descriptions, heading structure, alt text)
- Link management (broken links, external links, naked links)
- Configurable settings and thresholds
- Caching for performance (24-hour cache expiration)
- External link checking (optional, disabled by default, requires internet connection)

### Maintenance Tasks

- Keep ESLint configuration up-to-date (ESLint 9 flat config)
- Monitor Obsidian API changes (especially SettingGroup support since API 1.11.0+)
- Update reference materials as needed (UI Tweaker for settings patterns)
- Review and update SEO check thresholds based on best practices
- Maintain backward compatibility for older Obsidian versions (via `settings-compat.ts`)
- Keep dependencies up-to-date (TypeScript >=4.8.4 required for ESLint compatibility)
...
- **Error Handling**: Centralized error handling via `error-handler.ts` utility
- **Cache Management**: Performance optimization with intelligent cache management (24-hour expiration)
- **SettingGroup Usage**: Uses backward-compatible SettingGroup via `settings-compat.ts` utility
- **Conditional Visibility**: Settings with conditional visibility handled within their groups (e.g., `parentFolderSlugFilename` only shows when `useFilenameAsSlug` is enabled)

### Project-Specific References

- `.ref/plugins/obsidian-ui-tweaker/` - Reference implementation for SettingGroup usage and settings patterns. Used as a reference for the `settings-compat.ts` utility that provides backward-compatible SettingGroup support.
...
- `src/main.ts` - Main plugin class, initialization, lifecycle management, panel management
- `src/settings.ts` - Settings interface (`SEOSettings`) and default values (`DEFAULT_SETTINGS`)
- `src/settings-tab.ts` - Settings UI implementation (uses SettingGroup via compatibility utility)
- `src/utils/settings-compat.ts` - Backward-compatible SettingGroup utility for Obsidian API 1.11.0+ support
- `src/seo-checker.ts` - Main SEO checking logic and orchestration
- `src/checks/` - Modular check implementations:
  - `alt-text-check.ts` - Alt text validation for images, videos, embeds
  - `content-checks.ts` - Content length, reading level, keyword density checks
  - `duplicate-detection.ts` - Duplicate content and title/description detection
  - `external-link-checks.ts` - External link validation
  - `heading-checks.ts` - Heading hierarchy validation
  - `image-checks.ts` - Image filename validation
  - `link-checks.ts` - Broken link detection
  - `meta-checks.ts` - Meta title and description validation
- `src/ui/side-panel.ts` - Side panel view implementation for current note and vault-wide audits
- `src/ui/panel-manager.ts` - Panel lifecycle management
- `src/ui/panel-actions.ts` - Panel action handlers (refresh, scan, etc.)
- `src/ui/panel-renderer.ts` - Panel rendering logic
- `src/ui/results-display.ts` - Results display and sorting
- `src/utils/cache-manager.ts` - Cache management for scan results (24-hour expiration)
...
- **SettingGroup Implementation**: Uses backward-compatible SettingGroup via `settings-compat.ts` utility. The utility checks for API 1.11.0+ using `requireApiVersion('1.11.0')` and falls back to manual heading creation for older versions.

- **Settings Organization**: Settings are organized in logical groups:
  - **Global**: Directory configuration, note title preferences, title prefix/suffix, ignore patterns, ribbon icon
  - **Property names**: Keyword, description, title, slug property configurations
  - **Audit options**: All check toggles (title length, heading order, content length, alt text, links, etc.)
  - **Thresholds**: Minimum content length, keyword density ranges, duplicate content threshold

- **Conditional Visibility**: Settings with conditional visibility are handled within their groups. For example, `parentFolderSlugFilename` only shows when `useFilenameAsSlug` is enabled, and it's kept within the "Property names" group.

- **Performance Optimization**: 
  - Caching system with 24-hour cache expiration
  - First scan builds cache (slower), subsequent scans use cache (much faster)
  - Cache automatically expires after 24 hours
  - External link checking is optional and disabled by default (requires internet connection)

- **External Link Checking**: 
  - Optional feature that requires internet connection
  - Disabled by default for privacy/security
  - Can be enabled in settings if desired
  - Includes warning about performance impact for vault-wide checks

- **SEO Score System**: Uses weighted scoring (40-100 range):
...
- **Panel System**: 
  - Two panel types: current note audit and vault-wide audit
  - Panel manager handles lifecycle and file selection
  - Side panel can be refreshed and updated dynamically
  - Results can be sorted by issues, warnings, or filename

- **Error Handling**: Centralized error handling via `error-handler.ts` with `handleError()` and `withErrorHandling()` utilities for consistent error management throughout the plugin.
